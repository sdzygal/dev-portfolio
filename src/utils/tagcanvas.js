...
(function(){
    let i, j, abs = Math.abs, sin = Math.sin, cos = Math.cos, max = Math.max,
    ...
    function SetAlpha(c,a) {
        ...
        if(c[0] === '#') {
            ...
            p1 = c.lastIndexOf(',') + 1, p2 = c.indexOf(')'); //eslint-disable-line
...
// I think all browsers pass this test now...
    ...
    function TextSet(ctxt, font, colour, strings, padx, pady, shadowColour,
        ...
        shadowOffsets.length && (ctxt.shadowOffsetX = shadowOffsets[0], //eslint-disable-line
    ...
    function AddShadowToImage(i, w, h, scale, sc, sb, so) {
        ...
        so && (c.shadowOffsetX = so[0] * scale, c.shadowOffsetY = so[1] * scale); //eslint-disable-line
    ...
    Oproto.DrawColourImage = function(c,x,y,w,h,colour,tag,x1,y1) {
        ...
        if(ocanvas)
            ocanvas.width = fw, ocanvas.height = fh; //eslint-disable-line
        else
    ...
    Tproto.Clicked = function(e) {
        ...
        if(t != '' && t != '_self') {
            if(self.frames[t]) { //eslint-disable-line
                self.frames[t].document.location = h; //eslint-disable-line
            } else{
                try {
                    if(top.frames[t]) { //eslint-disable-line
                        top.frames[t].document.location = h; //eslint-disable-line
                        return;
                    }
                } catch(err) {
    ...
    TCproto.AnimateFixed = function() {
        ...
        if(this.fixedAnim) {
            ...
            fa = this.fixedAnim, t1 = TimeNow() - fa.t0, angle = fa.angle, //eslint-disable-line
    ...
    TCproto.AnimatePosition = function(w, h, t) {
        ...
        if(!tc.frozen && x >= 0 && y >= 0 && x < w && y < h) {
            s = tc.maxSpeed, r = tc.reverse ? -1 : 1; //eslint-disable-line
    ...
    TCproto.FindTag = function(t) {
        ...
        var srch, tgt, i;
        if(Defined(t.id))
            srch = 'id', tgt = t.id; //eslint-disable-line
        else if(Defined(t.text))
            srch = 'innerText', tgt = t.text; //eslint-disable-line
